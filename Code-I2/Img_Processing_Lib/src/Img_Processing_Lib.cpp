...
void Img_Processing_Lib::MaxFilter(unsigned char*_intImgData,unsigned char*_outImgData,int imgCols, int imgRows)
{
    int x,y,i,j,smax,n;
    int a[11][11];
    n=3;
        ...
        for(x=n/2;x<imgRows-n/2;x++)
    {
        ...
        for(j=-n/2;j<=n/2;j++)
            for(i=-n/2;i<=n/2;i++)
        {
            a[i+n/2][j+n/2]=*(_intImgData+x+i+(long)(y+j)*imgCols);
        }
        for(j=0;j<=n-1;j++)
        {
            for(i=0;i<n-1;i++)
...
void Img_Processing_Lib::MedianFilter(unsigned char*_intImgData , unsigned char*_outImgData,int imgCols, int imgRows)
{
    int x,y,i,j,z,n,a;
    int ar[121];
    n=7;
        ...
        for(x=n/2;x<imgCols-n/2;x++)
    {
        ...
        for(j=-n/2;j<=n/2;j++)
            for(i=-n/2;i<=n/2;i++)
        {
            ar[z] = *(_intImgData+x+i+(long)(y+j)*imgCols);
            z++;
        }
...
void Img_Processing_Lib::MinFilter(unsigned char*_intImgData , unsigned char*_outImgData,int imgCols, int imgRows)
{
    int x,y,i,j,smin,n;
    int a[11][11];
    n=5;
